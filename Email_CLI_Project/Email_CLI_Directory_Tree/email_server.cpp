...
EmailServer::EmailServer(const string& dataFile) {
    ...
    for (const auto& obj : data)
        emails.push_back(Email(obj["sender"], obj["subject"], obj["body"], obj["date"]));
}

bool EmailServer::getNewEmail(Email& out) {
    if (emails.empty() || (rand() % 4 != 0))
        return false;
    out = emails[rand() % static_cast<int>(emails.size())];
    return true;
}
